...
if [ "\$BOOTSTRAP_DEV" = "true" ]; then
  ...
  HAS_USERS=\$(kctl -n "\$NS" exec "deployment/postgres" -- env PGPASSWORD="\$POSTGRES_PASSWORD" psql -U dealer -d dealer -qtAc "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='auth' AND table_name='users');")
  if [ "\$HAS_USERS" != "t" ]; then
    echo "Применяю миграции (пустая БД)…"
    for f in \\
      000_schemas.up.sql 001_users.up.sql 002_roles.up.sql 003_customers.up.sql 004_vehicles.up.sql \\
