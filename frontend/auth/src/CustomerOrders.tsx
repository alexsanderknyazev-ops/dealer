...
export function CustomerOrders() {
  ...
  const [list, setList] = useState<api.CustomerOrder[]>([])
  ...
  useEffect(() => {
    ...
    setLoading(true)
    api
      .listCustomerOrders({ limit, offset: page * limit, status: statusFilter || undefined })
      .then((r) => {
        if (!cancelled) {
          setList(r.orders ?? [])
          setTotal(r.total)
              ...
              <TableBody>
                {list.map((o) => (
                  <TableRow key={o.id}>
                    ...
                    <TableCell>{o.issue_warehouse_name || '—'}</TableCell>
                    <TableCell>
                      <Badge variant="secondary">{STATUS_LABEL[o.status] || o.status}</Badge>
                    </TableCell>
                    <TableCell className="text-right">
                      <Link className="text-primary underline" to={`/customer-orders/${o.id}`}>Открыть</Link>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
