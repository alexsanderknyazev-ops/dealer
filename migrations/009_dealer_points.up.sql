...
ALTER TABLE vehicles.vehicles ADD COLUMN IF NOT EXISTS dealer_point_id UUID;
ALTER TABLE vehicles.vehicles ADD COLUMN IF NOT EXISTS legal_entity_id  UUID;
ALTER TABLE vehicles.vehicles ADD COLUMN IF NOT EXISTS warehouse_id     UUID;
CREATE INDEX IF NOT EXISTS idx_vehicles_dealer_point ON vehicles.vehicles (dealer_point_id);
CREATE INDEX IF NOT EXISTS idx_vehicles_legal_entity ON vehicles.vehicles (legal_entity_id);
CREATE INDEX IF NOT EXISTS idx_vehicles_warehouse ON vehicles.vehicles (warehouse_id);

-- Привязка запчастей к точке, юр. лицу и складу
ALTER TABLE parts.parts ADD COLUMN IF NOT EXISTS dealer_point_id UUID;
ALTER TABLE parts.parts ADD COLUMN IF NOT EXISTS legal_entity_id  UUID;
ALTER TABLE parts.parts ADD COLUMN IF NOT EXISTS warehouse_id     UUID;
CREATE INDEX IF NOT EXISTS idx_parts_dealer_point ON parts.parts (dealer_point_id);
CREATE INDEX IF NOT EXISTS idx_parts_legal_entity ON parts.parts (legal_entity_id);
CREATE INDEX IF NOT EXISTS idx_parts_warehouse ON parts.parts (warehouse_id);
