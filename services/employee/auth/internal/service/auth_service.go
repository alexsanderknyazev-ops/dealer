...
// RegisterClient регистрирует владельца авто (role=client).
func (s *AuthService) RegisterClient(ctx context.Context, email, password, name, phone string) (*domain.User, string, string, time.Time, error) {
	return s.register(ctx, email, password, name, phone, domain.ClientRole)
}

// Register регистрирует сотрудника, создаёт пару токенов и публикует событие в Kafka.
func (s *AuthService) Register(ctx context.Context, email, password, name, phone string) (*domain.User, string, string, time.Time, error) {
	return s.register(ctx, email, password, name, phone, domain.DefaultRole)
}

func (s *AuthService) register(ctx context.Context, email, password, name, phone, role string) (*domain.User, string, string, time.Time, error) {
	...
	if err := s.publishUserRegistered(ctx, user); err != nil {
		obslog.Default.Warn("kafka publish failed",
