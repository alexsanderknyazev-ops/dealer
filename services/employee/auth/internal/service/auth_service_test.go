...
func (pubFail) Publish(context.Context, []byte, []byte) error {
...
func TestRegister_PublishFailureStillSucceeds(t *testing.T) {
	_, rdb := testRedis(t)
	repo := &fakeUserRepo{byEmail: map[string]*domain.User{}, byID: map[uuid.UUID]*domain.User{}}
	s := NewAuthService(repo, rdb, pubFail{}, testCfg())
	ctx := context.Background()

	u, at, rt, exp, err := s.Register(ctx, "pub-fail@test", "pass12345", "U", "")
	if err != nil || at == "" || rt == "" || exp.IsZero() || u.Email != "pub-fail@test" {
		t.Fatalf("register should succeed despite publish error: err=%v", err)
	}
}
