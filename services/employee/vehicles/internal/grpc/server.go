...
func (s *Server) GetVehicleByVIN(ctx context.Context, req *vehiclesv1.GetVehicleByVINRequest) (*vehiclesv1.GetVehicleResponse, error) {
	if req.Vin == "" {
		return nil, status.Error(codes.InvalidArgument, "vin required")
	}
	v, err := s.svc.GetByVIN(ctx, req.Vin)
	if err != nil {
		if errors.Is(err, service.ErrNotFound) {
			return nil, status.Error(codes.NotFound, "vehicle not found")
		}
