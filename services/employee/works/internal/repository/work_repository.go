...
func (r *WorkRepository) Create(ctx context.Context, w *domain.Work) error {
	...
	`, w.ID, w.Code, w.Name, w.Category, w.LaborHours, w.UnitPrice, w.Notes, w.CreatedAt, w.UpdatedAt)
	return err
}
...
func (r *WorkRepository) GetByID(ctx context.Context, id uuid.UUID) (*domain.Work, error) {
	...
	err := r.pool.QueryRow(ctx, `
		SELECT id, code, name, category, labor_hours::text, unit_price::text, notes, created_at, updated_at
		FROM works WHERE id = $1
	`, id).Scan(&w.ID, &w.Code, &w.Name, &w.Category, &w.LaborHours, &w.UnitPrice, &w.Notes, &w.CreatedAt, &w.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &w, nil
...
func (r *WorkRepository) List(ctx context.Context, limit, offset int32, search, category string) ([]*domain.Work, int32, error) {
	...
	for rows.Next() {
		var w domain.Work
		if err := rows.Scan(&w.ID, &w.Code, &w.Name, &w.Category, &w.LaborHours, &w.UnitPrice, &w.Notes, &w.CreatedAt, &w.UpdatedAt); err != nil {
			return nil, 0, err
		}
		list = append(list, &w)
	}
...
func (r *WorkRepository) Update(ctx context.Context, w *domain.Work) error {
	_, err := r.pool.Exec(ctx, `
		UPDATE works
		SET code=$2, name=$3, category=$4, labor_hours=$5::numeric, unit_price=$6::numeric, notes=$7, updated_at=$8
		WHERE id=$1
	`, w.ID, w.Code, w.Name, w.Category, w.LaborHours, w.UnitPrice, w.Notes, w.UpdatedAt)
	return err
}
