...
func (h *Handler) auth(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if isWriteMethod(r.Method) && !hasAllowedRole(role, "admin", "manager", "parts_manager", "storekeeper") {
			log.Printf(
				"rbac deny: service=parts user_id=%s role=%s endpoint=%s action=%s reason=%s",
