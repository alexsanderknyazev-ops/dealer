...
import (
	...
	"github.com/dealer/dealer/client-statistics-service/internal/config"
	"github.com/dealer/dealer/client-statistics-service/internal/consumer"
	grpcserver "github.com/dealer/dealer/client-statistics-service/internal/grpc"
	"github.com/dealer/dealer/client-statistics-service/internal/repository"
...
const serviceName = "client-statistics-service"
...
func main() {
	...
	pool, err := postgres.NewPool(ctx, cfg.PostgresDSN, dbschema.ClientStatistics, dbschema.Public)
	if err != nil {
	...
	if len(cfg.KafkaBrokers) > 0 && cfg.KafkaBrokers[0] != "" {
		if cfg.KafkaReviewTopic != "" {
			reviewConsumer := kafka.NewConsumer(cfg.KafkaBrokers, cfg.KafkaReviewTopic, cfg.KafkaConsumerGroup)
			if reviewConsumer != nil {
				defer reviewConsumer.Close()
				go consumer.NewReviewWorker(reviewConsumer, svc, logger).Run(ctx)
				logger.Info("kafka consumer started", "topic", cfg.KafkaReviewTopic, "group", cfg.KafkaConsumerGroup)
			}
		}
		if cfg.KafkaRegistrationTopic != "" {
			regConsumer := kafka.NewConsumer(cfg.KafkaBrokers, cfg.KafkaRegistrationTopic, cfg.KafkaConsumerGroup)
			if regConsumer != nil {
				defer regConsumer.Close()
				go consumer.NewRegistrationWorker(regConsumer, svc, logger).Run(ctx)
				logger.Info("kafka consumer started", "topic", cfg.KafkaRegistrationTopic, "group", cfg.KafkaConsumerGroup)
			}
		}
	} else {
		logger.Warn("kafka consumers disabled for client statistics")
	}
	...
	clientstatsv1.RegisterClientStatisticsServiceServer(gsrv, grpcserver.NewServer(svc))
